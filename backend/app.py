...
def resolve_calibration_scales(reference_measurements):
    """
    Returns (scale_x_avg, scale_y_avg, scale_quality_dict) or (None, None, error_message).
    """
    ...
    for idx, ref in enumerate(reference_measurements):
        calibration_points = ref.get("calibrationPoints", [])
        known_length_cm = ref.get("knownLengthCm")

        if not isinstance(calibration_points, list) or len(calibration_points) != 2:
            return (
                None,
                None,
                f"Reference {idx + 1}: Exactly 2 calibration points are required.",
            )
        ...
        if direction == "horizontal":
            if abs_dx <= 0:
                return (
                    None,
                    None,
                    f"Reference {idx + 1}: horizontal reference has invalid deltaX.",
                )
            ...
            if abs_dy <= 0:
                return (
                    None,
                    None,
                    f"Reference {idx + 1}: vertical reference has invalid deltaY.",
                )
    ...
    if horizontal_count == 0 and vertical_count == 0:
        return (
            None,
            None,
            "No horizontal or vertical references found. Add at least one predominantly horizontal or vertical measurement.",
        )

    scale_x_avg = (
        sum(scale_x_horizontal) / horizontal_count if horizontal_count > 0 else None
    )
    scale_y_avg = (
        sum(scale_y_vertical) / vertical_count if vertical_count > 0 else None
    )
...
def calculate():
    ...
    reference_measurements = payload.get("referenceMeasurements", [])
    depth_mm = payload.get("depthMm")

    if len(polygon_points) < 3:
        return jsonify({"error": "At least 3 polygon points are required."}), 400

    try:
        depth_mm = float(depth_mm)
    except (TypeError, ValueError):
        return jsonify({"error": "depthMm must be a number."}), 400

    if depth_mm <= 0:
        return jsonify({"error": "depthMm must be > 0."}), 400

    scale_x_avg, scale_y_avg, scale_or_error = resolve_calibration_scales(
        reference_measurements
    )
    if scale_x_avg is None:
        return jsonify({"error": scale_or_error}), 400

    scale_quality = scale_or_error

    polygon_area_px = polygon_area_px2(polygon_points)
...
def calculate_wood():
    ...
    use_image_border_as_mold = payload.get("useImageBorderAsMold", True)
    mold_boundary_points = payload.get("moldBoundaryPoints", [])
    wood_boundary_points = payload.get("woodBoundaryPoints", [])
    wood_boundary_polygons = payload.get("woodBoundaryPolygons")
    cavity_polygons = payload.get("cavityPolygons", [])
    reference_measurements = payload.get("referenceMeasurements", [])
    main_pour_depth_mm = payload.get("mainPourDepthMm", payload.get("depthMm"))
    cavity_depths_mm = payload.get("cavityDepthsMm", [])
    ...
    try:
        ...
        return (
            jsonify(
                {
                    "error": "imageWidth, imageHeight, and mainPourDepthMm must be numbers."
                }
            ),
            400,
        )
    ...
    if image_width <= 0 or image_height <= 0:
        return jsonify({"error": "imageWidth and imageHeight must be > 0."}), 400

    if main_pour_depth_mm <= 0:
        return jsonify({"error": "mainPourDepthMm must be > 0."}), 400

    if not use_image_border_as_mold and len(mold_boundary_points) < 3:
        return jsonify({"error": "Mold boundary requires at least 3 points."}), 400
    ...
    if not isinstance(wood_boundary_polygons, list):
        return jsonify({"error": "woodBoundaryPolygons must be an array."}), 400

    if len(wood_boundary_polygons) == 0:
        return jsonify({"error": "At least one wood island is required."}), 400

    for idx, wood_polygon in enumerate(wood_boundary_polygons):
        if not isinstance(wood_polygon, list) or len(wood_polygon) < 3:
            return (
                jsonify({"error": f"Wood island {idx + 1}: at least 3 points are required."}),
                400,
            )

    if not isinstance(cavity_polygons, list):
        return jsonify({"error": "cavityPolygons must be an array."}), 400

    if not isinstance(cavity_depths_mm, list):
        return jsonify({"error": "cavityDepthsMm must be an array."}), 400

    if len(cavity_depths_mm) != len(cavity_polygons):
        return (
            jsonify(
                {
                    "error": "cavityDepthsMm must have one depth value per cavity polygon."
                }
            ),
            400,
        )
    ...
    for idx, cavity in enumerate(cavity_polygons):
        if not isinstance(cavity, list) or len(cavity) < 3:
            return (
                jsonify({"error": f"Cavity {idx + 1}: at least 3 points are required."}),
                400,
            )
    ...
    mold_area_px = (
        ...
        if use_image_border_as_mold
        else polygon_area_px2(mold_boundary_points)
    )
    ...
    for idx, cavity in enumerate(cavity_polygons):
        try:
            ...
            return (
                jsonify({"error": f"Cavity {idx + 1}: depth must be a number."}),
                400,
            )
        if depth_mm <= 0:
            return (
                jsonify({"error": f"Cavity {idx + 1}: depth must be > 0."}),
                400,
            )
    ...
    if main_resin_area_cm2 < 0:
        return (
            jsonify(
                {
                    "error": "Computed main resin area is negative. Check wood island traces."
                }
            ),
            400,
        )

    main_volume_liters = volume_liters_from_area_cm2(
        main_resin_area_cm2, main_pour_depth_mm
    )
